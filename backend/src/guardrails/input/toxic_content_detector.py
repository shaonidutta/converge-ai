"""
Toxic content detector guardrail for input.

This guardrail detects toxic, offensive, or inappropriate content in user input
...
class ToxicContentDetector(BaseGuardrail):
    """
    Detects toxic content in user input.
    ...
    async def check(self, text: str, context: Dict[str, Any]) -> GuardrailResult:
        """
        Check for toxic content in user input.
